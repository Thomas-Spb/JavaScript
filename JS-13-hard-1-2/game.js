...
(() => {
  ...
  const getFigure = lang => {
    ...
    if (lang === 'RUS') {
      botFigure = FIGURES_RUS[botFigureNumber];
    } else {
      botFigure = FIGURES_ENG[botFigureNumber];
    }
  ...
  const game = language => {
    language === '' ? (language = 'RUS') : (language = language);
    ...
    const start = () => {
      let computerMove = getFigure(language);
      ...
      if (language === 'RUS') {
        ...
        if (playerMove === null) {
          ...
          if (gameCancel) {
            alert(`
                Point PC: ${result.computer}
                Point player: ${result.player}`);
            return;
          } else {
            start();
          }
        } else if (playerMove[0] === 'r') {
          playerMove = 'rock';
        } else if (playerMove[0] === 's') {
          playerMove = 'scissors';
        ...
        if (computerMove === playerMove) {
          result.message = MESSAGE_ENG.tie;
        } else if (
          (computerMove === 'rock' && playerMove === 'paper') ||
          (computerMove === 'scissors' && playerMove === 'rock') ||
          (computerMove === 'paper' && playerMove === 'scissors')
        ) {
          result.message = MESSAGE_ENG.win;
          result.player++;
        } else {
          result.message = MESSAGE_ENG.lose;
          result.computer++;
        }

        alert(`Сhoosing a computer: ${computerMove}
          choosing a player: ${playerMove}
            Result: ${result.message}`);

        const playMore = confirm('Are we still playing?');
